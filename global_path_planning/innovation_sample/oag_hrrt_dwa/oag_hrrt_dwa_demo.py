...
INNOVATION_DIR = os.path.abspath(os.path.join(CURRENT_DIR, ".."))
PROJECT_ROOT = os.path.abspath(os.path.join(INNOVATION_DIR, "..", ".."))
DEFAULT_MAP_PATH = os.path.join(INNOVATION_DIR, "orchard_environment.npz")
...
class OAGHRRTDWAApp:
    def __init__(self, args):
        ...
        # 使用 local_path_planning 中的 DWA/TEB，并从 YAML 合并参数。
        planner_section = (self.config or {}).get('local_planner', {})
        self.local_planner_name = str(planner_section.get('type', 'dwa')).lower()
        if self.local_planner_name not in ('dwa', 'teb'):
            raise ValueError("local_planner.type 必须是 'dwa' 或 'teb'")
        ...
        yaml_values.update((self.config or {}).get(self.local_planner_name, {}))
