...
class HybridSampler:
    def __init__(
        ...
        self.initial_tangent_probability = tangent_probability
        ...
        self.tangent_probability = tangent_probability
    ...
    def active_obstacle_passed(self, pose):
        ...
        if target_distance <= 1.2:
            return True

        if (
            guidance.ellipse_a is None
            or guidance.ellipse_b is None
            or guidance.ellipse_a <= 1e-6
            or guidance.ellipse_b <= 1e-6
        ):
            return True
    ...
    def _normalize_probabilities(self):
        ...
        if total_guided >= 1.0:
            ...
            self.tangent_probability = values[1] * scale
            self.corridor_probability = values[2] * scale
            self.rectangle_probability = values[3] * scale
    ...
    def update_probability(
        ...
        if iteration < 500:
            ...
            self.rectangle_probability = max(
                0.0, 0.50 - self.tangent_probability
            )
        ...
        if self.no_improvement_count > 300:
            ...
            if guided_total > 1e-12:
                ...
                self.tangent_probability *= scale
    ...
    def sample(self):
        ...
        threshold += self.tangent_probability
    ...
    def tangent_sample(self):
        """在切向目标附近随机采样，避免反复返回完全相同的状态。"""
        target_x, target_y = self.sampling_target
        anchor_x = self.goal_rectangle.anchor_x if self.goal_rectangle else self.goal.x
        anchor_y = self.goal_rectangle.anchor_y if self.goal_rectangle else self.goal.y
        ...
        for _ in range(self.max_sample_attempts):
            along_offset = self.random.gauss(0.0, 0.80)
            lateral_offset = self.random.gauss(0.0, 0.50)
            x = target_x + along_offset * ux + lateral_offset * nx
            y = target_y + along_offset * uy + lateral_offset * ny
            ...
            yaw = self.normalize_angle(
                math.atan2(target_y - y, target_x - x)
                + self.random.gauss(0.0, self.heading_std * 0.5)
            )
            return x, y, yaw
