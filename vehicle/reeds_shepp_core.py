...
def left_right_x_left(x, y, phi):
    ...
    if u1 <= 4.0:
        ...
        asin_arg = max(-1, min(1, 2 * math.sin(u) / u1))
...
def reeds_shepp_path_planning(
    ...
    maxc: float, step_size: float = 0.2,
):
    """返回最短 Reeds-Shepp 路径及其逐点方向和分段信息。"""
    paths = calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size)

    if not paths:
        return None, None, None, None, None, None

    path = min(paths, key=lambda p: p.L)
    return path.x, path.y, path.yaw, path.directions, path.ctypes, path.lengths
