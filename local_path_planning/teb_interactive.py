...
class InteractiveTEBTest:
    ...
    def _planning_job(config, bounds, start_pos, goal_pos, obstacles):
        ...
        # 完整 16 m 演示轨迹比实时控制窗口更长，给时间带更充足的收敛空间。
        config.max_iterations = max(config.max_iterations, 200)
        config.max_accel = max(config.max_accel, 3.0)
        config.max_decel = max(config.max_decel, 3.0)
        num_points = 10
        global_path = []
