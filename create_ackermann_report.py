...
story += [P('2.1 场景合理性修正','H'),P('旧生成器会在任意位置放置 3 x 5 栅格箱体，可能封死行间通道或在起终点附近形成车辆不可达地图。修正版将树干作为行结构边界，将枝条/箱体限制在走廊边缘，显式清空起终点 headland，并保持一条满足 NWD01 车宽的中央连通带。难度由边缘障碍数量与侵入程度、动态横穿障碍数量共同定义，而不是通过随机封路制造。对种子 4000-4011 的预检表明，12/12 个简单、中等和困难场景均存在通过 footprint 扫掠检查的 SE(2) 全局走廊。','B'),P('修正场景后完整 Ours 的 6 场景闭环成功率仍为 33.3%，且碰撞为 0。因此剩余失败主要来自 MPPI 收敛、动态风险权重或仿真时限，而不是全局地图不可达。','B')]
...
if two:
 ...
 story += [T(tbl2,[43*mm,19*mm,18*mm,22*mm,23*mm,23*mm,25*mm]),P(f'6 个独立场景的初步结果显示，完整 Ours 成功率为 {float(full["success_rate"])*100:.1f}%，footprint 平均碰撞为 {float(full["mean_collisions"]):.1f}。去除全局风险项后平均碰撞增至 {float(ngr["mean_collisions"]):.1f}，说明风险代价对扫掠体安全具有作用；去除动态预测后成功率为 {float(ndp["success_rate"])*100:.1f}%，但失败场景中的平均碰撞显著增加，提示不能以牺牲安全换取到达率。无恢复变体与完整方法相同，是因为本批场景未触发恢复条件。','B')]
